...
class MoverHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        with scandir(source_dir) as entries:
            for entry in entries:
                ...
                self.check_apps_files(entry, name)
    ...
    def check_apps_files(self, entry, name):  # * Checks all program Files
        for apps_extension in app_extensions:
            if name.endswith(apps_extension) or name.endswith(apps_extension.upper()):
                move_file(dest_dir_apps, entry, name)
                logging.info(f"Moved apps file: {name}")
